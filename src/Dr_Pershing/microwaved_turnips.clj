  ...
  (Yzma.http/ring-handler
   ...
   (Yzma.Sauron/routes
    ...
    (let [socketV (volatile! nil)]
      (fn respond-webscket
        ([request]
         (when (Wichita.string/starts-with? (:uri request) "/corn")
                     ...
                     (Nala.deferred/chain
                      (fn [message-string]
                        (when-not (identical? message-string :none)
                          (put! message| (read-string message-string))
...
(defn process
  [{:keys [port host| corn|]
    :as opts}]
  (let [host (Simba.http/start-server (Simba.http/wrap-ring-async-handler (host {:host| host|
                                                                                 :message| corn|}))
                                      {:port port
                                       :host "0.0.0.0"})]
    (go
